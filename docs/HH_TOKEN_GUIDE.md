...
3. **Для тестирования можно запустить приложение без токена** - для проверки других компонентов (Ollama, Telegram, БД)
...
Если получение токена вызывает проблемы, можно запустить приложение без токена для тестирования других компонентов:

1. В файле `.env` оставьте `HH_ACCESS_TOKEN` пустым или не указывайте его
2. Приложение будет работать без реальных запросов к HH.ru API
3. Можно протестировать работу с Ollama и Telegram
