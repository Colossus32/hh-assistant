...
interface VacancyRepository : JpaRepository<Vacancy, String> {
    ...
    /**
     * Получает все вакансии с пагинацией, отсортированные по дате получения (новые первыми).
     */
    fun findAllByOrderByFetchedAtDesc(pageable: Pageable): Page<Vacancy>
    ...
    /**
     * Получает список вакансий, добавленных за последние 24 часа
     * @param cutoffTime Время отсечки (24 часа назад)
     * @return Список вакансий, отсортированных по fetchedAt
     */
    @Query(
        """
        SELECT v FROM Vacancy v 
        WHERE v.fetchedAt >= :cutoffTime
        ORDER BY v.fetchedAt DESC
    """,
    )
    fun findVacanciesAddedInLast24Hours(cutoffTime: java.time.LocalDateTime): List<Vacancy>
}
