...
class VacancyStatusService(
    ...
    override fun updateVacancyStatus(updatedVacancy: Vacancy) {
        try {
            ...
            if (oldStatus != newStatus && !VacancyStatus.canTransition(oldStatus, newStatus)) {
                log.warn(
                    "⚠️ [StatusService] Unexpected status transition for ${updatedVacancy.id}: $oldStatus -> $newStatus",
                )
            }
