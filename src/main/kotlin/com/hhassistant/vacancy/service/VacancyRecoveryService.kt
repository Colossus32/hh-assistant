...
class VacancyRecoveryService(
    ...
    private val skillExtractionService: com.hhassistant.analysis.service.SkillExtractionService,
    ...
    private suspend fun processOldSkippedVacancies(): Triple<Int, Int, Int> {
        ...
        oldVacancies.forEach { vacancy ->
            try {
                ...
                // Шаг 3: Нет анализа - проверяем URL (пропускаем для вакансий из Telegram каналов)
                val urlCheckResult = if (vacancy.isFromTelegram()) {
                    log.debug("[VacancyRecovery] Skipping URL check for Telegram channel vacancy ${vacancy.id}")
