package com.hhassistant.analysis.service
...
import com.hhassistant.analysis.repository.SkillRepository
...
class SkillExtractionService(
     ...
     */
    @Loggable
    override suspend fun extractAndSaveSkills(
        vacancy: Vacancy,
        keySkillsFromApi: List<KeySkillDto>?,
    ): List<Skill> {
        ...
        // Шаг 2: Если навыков недостаточно - дополнить через LLM из описания (suspend - LLM вызов)
        ...
        // Шаг 3: Нормализация навыков (CPU-bound, без suspend)
        ...
        // Шаг 4-6: Сохранение в БД в отдельном blocking-методе с транзакцией
        // (без suspend points внутри транзакции - устраняет Problem 4 из PROJECT_REVIEW)
        return withContext(Dispatchers.IO) {
            saveSkillsToDatabaseBlocking(vacancy, normalizedSkills)
        }
    }

    /**
     * Сохраняет навыки в БД в рамках одной транзакции.
     * Вызывается через withContext(Dispatchers.IO) из suspend-метода — без suspend points внутри транзакции.
     */
    @Transactional(rollbackFor = [Exception::class])
    private fun saveSkillsToDatabaseBlocking(vacancy: Vacancy, normalizedSkills: List<String>): List<Skill> {
            ...
            val response = ollamaRetry.executeSuspendFunction {
                ollamaCircuitBreaker.executeSuspendFunction {
                    ollamaClient.chat(
                        ...
                        taskType = com.hhassistant.monitoring.service.OllamaTaskType.SKILL_EXTRACTION,
                    )
                }
            }
