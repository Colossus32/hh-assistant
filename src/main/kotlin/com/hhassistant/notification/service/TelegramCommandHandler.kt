...
class TelegramCommandHandler(
    ...
    suspend fun handleCommand(chatId: String, text: String) {
        ...
        try {
            val response = when {
                ...
                text == "/restart_recent_vacancies" -> handleRestartRecentVacanciesCommand(chatId)
                ...
                text == "/reset_pagination" -> handleResetPaginationCommand()
    ...
    private fun handleStartCommand(): String {
        return buildString {
            ...
            appendLine("   /restart_recent_vacancies - Перезапустить вакансии за последние 24 часа")
    ...
    /**
     * Обрабатывает команду /reset_pagination - сбрасывает прогресс пагинации
     */
    private suspend fun handleResetPaginationCommand(): String {
        return try {
            val response = webClient.post()
                .uri("$apiBaseUrl/api/pagination/reset-all")
                .retrieve()
                .bodyToMono<Map<String, Any>>()
                .awaitSingle()

            val total = response["total"] as? Int ?: 0
            val successful = response["successful"] as? Int ?: 0
            val failed = response["failed"] as? Int ?: 0
            @Suppress("UNCHECKED_CAST")
            val results = (response["results"] as? List<*>)?.mapNotNull { it as? Map<String, Any> } ?: emptyList()

            buildString {
                appendLine("🔄 <b>Сброс пагинации завершен</b>")
                appendLine()
                appendLine("Всего конфигураций: <b>$total</b>")
                appendLine("✅ Успешно: <b>$successful</b>")
                if (failed > 0) {
                    appendLine("❌ Ошибок: <b>$failed</b>")
                }
                appendLine()
                appendLine("💡 Следующий фетч вакансий начнется с первой страницы")
                appendLine()
                if (failed > 0) {
                    appendLine("<b>Ошибки:</b>")
                    results.forEach { result ->
                        if (result["success"] == false) {
                            appendLine("  • ${result["configKey"]}: ${result["message"]}")
                        }
                    }
                    appendLine()
                }
                appendLine("💡 Новые вакансии должны появиться при следующем автоматическом фетче")
            }
        } catch (e: Exception) {
            log.error("❌ [TelegramCommand] Error resetting pagination: ${e.message}", e)
            "❌ Ошибка при сбросе пагинации: ${e.message ?: "Неизвестная ошибка"}"
        }
    }
    ...
    private fun handleHelpCommand(chatId: String): String {
        ...
        return buildString {
            ...
            appendLine()
            appendLine("<b>/restart_recent_vacancies</b> - Перезапустить вакансии, добавленные за последние 24 часа")
            appendLine("   Сбрасывает статус вакансий на NEW и добавляет их в очередь обработки")
            appendLine()
            ...
            appendLine()
            appendLine("<b>/reset_pagination</b> - Сбросить прогресс пагинации поиска")
            appendLine("   Сбрасывает прогресс для всех конфигураций поиска (Java, Kotlin)")
            appendLine("   Следующий фетч вакансий начнется с первой страницы")
            appendLine()
    ...
    /**
     * Обрабатывает команду /restart_recent_vacancies
     */
    private suspend fun handleRestartRecentVacanciesCommand(chatId: String): String {
        return try {
            log.info("[TelegramCommand] Restarting recent vacancies (last 24 hours)")
            
            // Запускаем перезапуск вакансий в фоне
            backgroundScope.launch {
                try {
                    val (restartedCount, vacancyIds) = vacancyService.restartRecentVacancies()
                    
                    // Добавляем перезапущенные вакансии в очередь обработки
                    if (restartedCount > 0 && vacancyIds.isNotEmpty()) {
                        val enqueuedCount = vacancyProcessingQueueService.enqueueBatch(vacancyIds)
                        
                        val message = buildString {
                            appendLine("✅ <b>Перезапуск вакансий завершен</b>")
                            appendLine()
                            appendLine("📊 <b>Статистика:</b>")
                            appendLine("   • Перезапущено вакансий: $restartedCount")
                            appendLine("   • Добавлено в очередь: $enqueuedCount")
                            appendLine()
                            appendLine("Вакансии будут обработаны в ближайшее время.")
                        }
                        
                        sendMessageSafely(chatId, message)
                    } else {
                        val message = buildString {
                            appendLine("ℹ️ <b>Нет вакансий для перезапуска</b>")
                            appendLine()
                            appendLine("Все вакансии за последние 24 часа:")
                            appendLine("   • Уже обрабатываются (NEW/QUEUED)")
                            appendLine("   • В архиве (IN_ARCHIVE)")
                            appendLine("   • Отклонены валидатором (REJECTED_BY_VALIDATOR)")
                            appendLine("   • Или в других терминальных статусах")
                            appendLine()
                            appendLine("Это нормальная ситуация, если за сегодня не появилось новых подходящих вакансий.")
                        }
                        sendMessageSafely(chatId, message)
                    }
                } catch (e: Exception) {
                    log.error("[TelegramCommand] Error restarting recent vacancies: ${e.message}", e)
                    sendMessageSafely(chatId, "❌ Ошибка при перезапуске вакансий: ${e.message ?: "Неизвестная ошибка"}")
                }
            }
            
            // Возвращаем сообщение сразу
            "🔄 <b>Запущен перезапуск вакансий</b>\n\nОбработка выполняется в фоне. Результат будет отправлен, когда завершится."
        } catch (e: Exception) {
            log.error("[TelegramCommand] Error starting restart: ${e.message}", e)
            "❌ Ошибка при запуске перезапуска вакансий: ${e.message ?: "Неизвестная ошибка"}"
        }
    }
